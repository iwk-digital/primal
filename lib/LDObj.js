...
export default class LDObj {
  ...
  async prepare() {
    try {
      ...
      if (DEBUG) console.log("Compacting with context: ", context);
      this.compacted = await jsonld.compact(this.expanded, context);
      this.validate();
      await this.processTargets();
      if (DEBUG) console.log("Processed targets! Object: ", this);
      return this;
    } catch (err) {
      console.error("Failed to prepare LD object", err);
      throw new Error(err);
    }
  }
  ...
  async processTargets() {
    if (DEBUG) console.log("Process targets called: ", this.expanded["@id"]);
      ...
      if (this.expanded.hasOwnProperty(predicate)) {
        if (this.expanded[predicate].length > 0) {
          for (const target of this.expanded[predicate]) {
            if (DEBUG) console.log("Processing target: ", target);
            if (target["@id"]) {
              const url = new URL(target["@id"]);
              if (url.protocol === "http:" || url.protocol === "https:") {
                // strip the URL of any fragment
                const strippedUrl = url.origin + url.pathname;
                if (DEBUG) console.log("Stripped URL: ", strippedUrl);
                // remember the pointing resource's label, if one is given
                let label =
                  ns.rdfs("label") in this.expanded
                    ? this.expanded[ns.rdfs("label")][0]["@value"]
                    : Graph.labelify(strippedUrl);
                ...
                if (await isAudio(strippedUrl)) {
                  if (!this.targets[strippedUrl]) {
                    ...
                    this.targets[strippedUrl] = {
                      ...
                      label: label,
                      timeline:
                        ns.tl("onTimeLine") in target
                          ? target[ns.tl("onTimeLine")][0]["@id"]
                          : null,
                      peaks:
                        ns.ssv("peaks") in this.expanded
                          ? this.expanded[ns.ssv("peaks")][0]["@value"]
                          : null,
                    };
                  }
                  // add the fragment to the set of fragments for this URL
                  ...
                  if (
                    ...
                    strippedUrl.endsWith(".jsonld")
                  ) {
                    // if target smells like a JSON-LD file, mark it for traversal
  ...
  getAudioTargets() {
    // return the MEI targets
    const t = Object.keys(this.targets).filter((key) => {
      return this.targets[key].type === "Audio";
    });
    let audioTargets = [];
    for (const key of t) {
      audioTargets.push({
        url: key,
        fragments: this.targets[key].fragments,
        timeline: this.targets[key].timeline,
        peaks: this.targets[key].peaks,
        label: this.targets[key].label,
      });
    }
    return audioTargets;
