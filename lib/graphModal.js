...
export default class GraphModal {
  ...
  handleTouchEnd(e) {
    ...
    if (this.isPinching || this.hasDragged) {
      e.preventDefault();
    }
  }
  ...
  handlePointerDown(e) {
    ...
    if (this.isInteractiveTarget(e.target)) return;
    this.elements.modalContent?.setPointerCapture?.(e.pointerId);
    ...
    // Do not prevent default so click events can still fire
  }
  ...
  handlePointerUp(e) {
    this.elements.modalContent?.releasePointerCapture?.(e.pointerId);
    ...
    // Allow default so clicks can be generated
  }
  ...
  handleClick(e) {
    ...
    // Close modal when a clickable graph node is activated
    const target = e.target;
    const isClickable = this.isClickableNode(target);
    if (isClickable && this.isModalOpen()) {
      // Allow Mermaid's click handler to run, then close the modal
  ...
  isClickableNode(node) {
    ...
    // Walk up the DOM/SVG tree to find a clickable class within the modal content
    let el = node;
    while (el && el !== this.elements.modalContent) {
