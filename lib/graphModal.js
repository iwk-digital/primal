...
export default class GraphModal {
  ...
  zoomAroundCenter(newZoom, animated = false) {
    ...
    // Adjust pan offset to keep the center point stationary
  ...
  handleWheel(e) {
    ...
    // The formula: new_offset = mouse_pos - zoom_ratio * (mouse_pos - old_offset)
    this.panOffset.x = mouseX - zoomRatio * (mouseX - this.panOffset.x);
    this.panOffset.y = mouseY - zoomRatio * (mouseY - this.panOffset.y);
