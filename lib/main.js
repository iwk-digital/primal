...
const regions = RegionsPlugin.create(); // wavesurfer regions plugin
  ...
  for (const target of Object.keys(audioTargets)) {
    ...
    ws.on("decode", () => {
      ...
      audioTargets[target].forEach((fragment) => {
        ...
        let start = parseInt(frags[0]);
        let end = parseInt(frags[1]);
        console.log("Adding region: ", start, end);
        regions.addRegion({
          start: start,
          end: end,
          content: "",
          color: "rgba(255, 255, 222, 0.5)",
          drag: false,
          resize: false,
        });
      });
