...
export const versionDate = "5 March 2026";
...
async function traversalsComplete() {
  ...
  if (objUrl.href in Graph.registry) {
    ...
    // Include only blank nodes directly associated with the subject (e.g., textual bodies)
    const blankLookupKeys = [
      objUrl.href,
      Graph.registry[objUrl]?.expanded?.["@id"],
      Graph.registry[objUrl]?.compacted?.["@id"],
    ].filter(Boolean);

    const addedBlankIds = new Set();
    blankLookupKeys.forEach((k) => {
      const subjectBlanks = Graph.blanks[k] || [];
      subjectBlanks.forEach((b) => {
        if (addedBlankIds.has(b)) return;
        visgraph += "\n" + b + "\n";
        addedBlankIds.add(b);
      });
    });

    // Include tracked textual-body nodes for the subject that are blanks (defensive filter)
    const textNodes = Graph.textNodesBySubject?.[objUrl.href];
    if (textNodes) {
      textNodes.forEach((tid) => {
        if (!tid || /^https?:\/\//i.test(tid)) return; // keep subgraph to blanks/locals
        if (addedBlankIds.has(tid)) return;
        visgraph += "\n" + tid + "\n";
        addedBlankIds.add(tid);
      });
    }
    visgraph += "\n end\n";
