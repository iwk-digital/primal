...
export const versionDate = "20 May 2025";
...
async function renderAudioTargets(audioTargets) {
  ...
  for (const target of Object.keys(audioTargets)) {
    ...
    audioLabel.innerHTML = `<span>${target}</span>`;
  ...
  for (const target of Object.keys(audioTargets)) {
      ...
      if ("timeline" in audioTargets[target]) {
        ...
        if (signal && NS.ssv("peaks") in signal) {
          ...
          if (peaks) {
            ...
            wavesurfers[target].regions = RegionsPlugin.create(); // wavesurfer regions plugin
            wavesurfers[target].ws = WaveSurfer.create({
              container: "#" + CSS.escape(audioTargets[target]["timeline"]),
              waveColor: "#b4becf",
              progressColor: "#163c7a",
              plugins: [wavesurfers[target].regions],
              mediaControls: true,
            ...
            // now we can load in the precomputed peaks
            // Create a dummy audio context
            const audioContext = new (window.AudioContext ||
              window.webkitAudioContext)();
            const dummyAudioBuffer = audioContext.createBuffer(
              1,
              peaks.length,
              audioContext.sampleRate
            );

            // Load the dummy audio buffer
            wavesurfers[target].load(audioContext, dummyAudioBuffer);

            wavesurfers[target].backend.buffer = peaks;

            // Redraw the waveform
            wavesurfers[target].drawBuffer();
            success = true;
          }
        }
      }
    ...
    if (success) {
      ...
      wavesurfers[target].ws.on("decode", (e) => {
        ...
        audioTargets[target].fragments.forEach((fragment) => {
