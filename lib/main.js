...
const regions = WaveSurfer.Regions.create(); // wavesurfer regions plugin
...
async function renderAudioTargets(audioTargets) {
  ...
  for (const target of Object.keys(audioTargets)) {
    ...
    wavesurfer.on("region-click", (region, e) => {
      ...
      regions.clearRegions(wavesurfer);
      // add highlight to the clicked region
      regions.updateRegion(wavesurfer, region.id, {
    ...
    wavesurfer.on("play", () => {
      ...
      let currentRegion = regions.getCurrentRegion(wavesurfer);
      if (currentRegion) {
        regions.updateRegion(wavesurfer, currentRegion.id, {
          color: "rgba(0, 255, 0, 0.5)",
        });
      }
    });
