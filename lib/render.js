...
export async function renderAudioTargets(audioTargets) {
  for (const target of Object.keys(audioTargets)) {
    ...
    if (haveAudio) {
      ...
      // Resolve peaks data: inline from the selection, or via the timeline
      let peaks = null;
      let durationToUse = null;
      let containerId = null;
      if (audioTargets[target].peaks) {
        // Inline peaks: JSON string with {peaks: [...], duration: ...}
        try {
          const parsed = JSON.parse(audioTargets[target].peaks);
          peaks = parsed.peaks;
          durationToUse = parsed.duration;
          containerId = target;
        } catch (e) {
          console.warn("Failed to parse inline peaks data:", e);
        }
        ...
        if (signal && NS.ssv("peaks") in signal) {
          const peaksUri = signal[NS.ssv("peaks")][0]["@id"];
          containerId = signal["@id"];
          peaks = await requestAsJsonLd(peaksUri);
          if (peaks) {
            ...
            durationToUse = (peaks.length * segmentSize) / sampleRate;
          }
        }
      }
      if (peaks && containerId) {
